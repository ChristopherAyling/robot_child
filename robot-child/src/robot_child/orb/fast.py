...
@njit
def make_corner_points(
    img: np.ndarray, grid: IntPairs, test_points: IntPairs, threshold: int
) -> IntPairs:
    n = 3
    corner_points = []
    for i, j in grid:
...
def make_orientations(
    img: np.ndarray, corner_points: IntPairs, margin: int
) -> list[float]:
    """
...
def fast_detection(img, grid, test_points, threshold, margin):
    corner_points = make_corner_points(
        img, grid=grid, test_points=test_points, threshold=threshold
    )
